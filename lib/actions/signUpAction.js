...
export async function signUpAction(prevState, formData) {
  // Convert FormData to object and construct phone field
  const formDataObj = Object.fromEntries(formData);

  // Construct phone object from separate fields
  if (formDataObj.dialCode || formDataObj.number) {
    formDataObj.phone = JSON.stringify({
      dialCode: formDataObj.dialCode || "",
      number: formDataObj.number || "",
    });
  }

  // Remove individual phone fields to avoid conflicts
  delete formDataObj.dialCode;
  delete formDataObj.number;

  console.log(formDataObj);

  const signUpValidation = validateSignupFormData(formDataObj);
